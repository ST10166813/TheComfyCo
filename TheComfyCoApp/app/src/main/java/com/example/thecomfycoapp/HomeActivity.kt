...
class HomeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppCompatDelegate.setDefaultNightMode(prefs.getInt("theme_mode", AppCompatDelegate.MODE_NIGHT_NO))
        ...
        arrow = DrawerArrowDrawable(this).apply {
            color = MaterialColors.getColor(toolbar, com.google.android.material.R.attr.colorOnSurface)
        }
        ...
        drawerLayout.addDrawerListener(object : DrawerLayout.SimpleDrawerListener() {
            ...
            override fun onDrawerClosed(drawerView: View) { if (isAtTopLevel()) arrow.progress = 0f }
            override fun onDrawerOpened(drawerView: View) { if (isAtTopLevel()) arrow.progress = 1f }
        })

        // ----- Bottom nav
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.menu_home -> { safeNavigate(homeDestId); true }
                R.id.menu_cart -> { safeNavigate(cartDestId); true }
                else -> false
            }
        }
        ...
        fabBtn.setOnClickListener { /* Optional: wishlist or quick action */ }
        ...
        settingsItem.setOnClickListener { safeNavigate(settingsDestId); closeDrawer() }
        ...
        navController.addOnDestinationChangedListener { _, destination, _ ->
            ...
            if (isAtTopLevel(destination.id)) {
                ...
                arrow.progress = 0f
                toolbar.setNavigationOnClickListener { if (!drawerLayout.isDrawerOpen(GravityCompat.START)) drawerLayout.openDrawer(GravityCompat.START) }
            } else {
                drawerLayout.setDrawerLockMode(DrawerLayout.LOCK_MODE_LOCKED_CLOSED, GravityCompat.START)
                arrow.progress = 1f
                toolbar.setNavigationOnClickListener { navController.navigateUp(appBarConfig) }
            }
        ...
        FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
            if (task.isSuccessful) sendTokenToBackend(task.result ?: "")
            else Log.e("FCMToken", "Failed to get token")
        }
    }

    private fun sendTokenToBackend(token: String) {
        val authToken = getSharedPreferences("auth", Context.MODE_PRIVATE).getString("token", null) ?: return
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("FCMToken", if (response.isSuccessful) "Token sent successfully" else "Failed: ${response.code()} ${response.message()}")
            } catch (e: Exception) {
    ...
    private fun closeDrawer() { if (drawerLayout.isDrawerOpen(GravityCompat.START)) drawerLayout.closeDrawer(GravityCompat.START) }

    private fun safeNavigate(destId: Int) {
        val current = navController.currentDestination?.id
        if (current != destId) try { navController.navigate(destId); syncBottomNavSelection(destId) } catch (_: Exception) {}
    }
